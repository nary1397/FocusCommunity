											...
											<div class="btn-list">
													<c:if test="${cusbodVO.cusid eq sessionid}">
														<button class="btn btn-info btn-left btn-sl-sm mb-5"
															type="button" onclick="location.href='/qna/modify?pageNum=${pageNum}&keyword=${keyword}&type=${type}&csnum=${cusbodVO.csnum}&cusid=${usbodVO.cusid}&ctype=${cusbodVO.ctype}'">
													...
													<button class="btn btn-success btn-left btn-sl-sm mb-5"
														type="button" onclick="location.href='/qna/replyWrite?crerf=${ cusbodVO.crerf }&crelv=${ cusbodVO.crelv }&cresq=${ cusbodVO.cresq }&pageNum=${ pageNum }&keyword=${keyword}&type=${type}'">
														<i class="bx bx-note"></i> 답글
														</button>
													</c:if>
												<button class="btn btn-dark btn-right btn-sl-sm mb-5"
													type="button" onclick="location.href='/qna/list?pageNum=${pageNum}&keyword=${keyword}&type=${type}'">
	...
	***********************************-->
	...
	<script>
		...
		function remove2(event, data, cusid){
			...
			swal({ title: "댓글 삭제", 
				...
				if (result.value == true) {
					location.href = '/qna/replyRemove?pageNum=${pageNum}&keyword=${keyword}&type=${type}&csnum=${cusbodVO.csnum}&cusid=' + cusid + '&conum='+data;
				}
			});
		}
		...
		function reply2(event, cusid, conum){
			...
			location.href = '/qna/replyModify?pageNum=${pageNum}&keyword=${keyword}&type=${type}&csnum=${cusbodVO.csnum}&cusid=' + cusid + '&ctype=${cusbodVO.ctype}&cnten='+cnten + '&conum='+conum;
		}
		...
		function reply3(event, cusid, crerf, crelv, cresq){
			...
			location.href = '/qna/replyRe?pageNum=${pageNum}&keyword=${keyword}&type=${type}&csnum=${cusbodVO.csnum}&cusid=' + cusid + '&ctype=${cusbodVO.ctype}&cnten='+cnten + '&crerf=' + crerf + '&crelv=' + crelv + '&cresq=' + cresq;
		}
		...
		function reply(event, cusid){
			...
			location.href = '/qna/reply?pageNum=${pageNum}&keyword=${keyword}&type=${type}&csnum=${cusbodVO.csnum}&cusid=' + cusid + '&${cusbodVO.ctype}&cnten='+cnten;
		}
		...
		$('#btnRemove').on('click', function(){
			...
			swal({ title: "게시글삭제", 
				...
				if (result.value == true) {
	  				location.href = '/qna/remove?csnum=${ cusbodVO.csnum }&ctype=${cusbodVO.ctype}&pageNum=${ pageNum }&keyword=${keyword}&type=${type}';
				}
			});
		});
	</script>
