				...
				<div class="col-lg-12">
                	<div class="card">
						...
						<c:if test="${ not empty sessionid }">
							<div>
				                <a href="/qna/write?pageNum=${pageMaker.cri.pageNum}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}" class="btn btn-sm btn-primary float-right me-sm-3">
										...
										<c:choose>
											<c:when test="${pageMaker.totalCount gt 0}">
												<c:forEach var="cusbod" items="${cusbodList}" varStatus="i">
													...
													<tr class="datatr" onclick="location.href='/qna/content?csnum=${cusbod.csnum}&cusid=${cusbod.cusid}&ctype=${cusbod.ctype}&pageNum=${pageMaker.cri.pageNum}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}'">
                                    ...
                                    <ul class="pagination pagination-xs pagination-circle">
                                   		<c:if test="${pageMaker.prev eq true}">
	                                        <li class="page-item page-indicator">
	                                            <a class="page-link" href="/qna/list?pageNum=${pageMaker.startPage - 1}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}#board"><i class="icon-arrow-left"></i></a>
	                                        </li>
                                   		</c:if>
                                   		<c:forEach var="i" begin="${pageMaker.startPage}" end="${pageMaker.endPage}" step="1">
                                        	<li class="page-item ${pageMaker.cri.pageNum eq i ? 'active' : ''}"><a class="page-link" href="/qna/list?pageNum=${ i }&type=${ pageMaker.cri.type }&keyword=${ pageMaker.cri.keyword }">${i}</a></li>
                                   		</c:forEach>
                                   		<c:if test="${pageMaker.next eq true}">
	                                        <li class="page-item page-indicator">
	                                            <a class="page-link" href="/qna/list?pageNum=${pageMaker.endPage + 1}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}#board"><i class="icon-arrow-right"></i></a>
	                                        </li>
                                   		</c:if>
                                    </ul>
                                </nav>
