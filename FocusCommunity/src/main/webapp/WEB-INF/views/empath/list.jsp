									...
									<tbody>
										<c:choose>
											<c:when test="${pageMaker.totalCount gt 0}">
												<c:forEach var="cusbod" items="${cusbodList}">
													...
													<c:choose>
														<c:when test="${cusbod.ctype eq 'I'}">
															<c:set var="trdata" value="location.href='/info/content?cusid=${cusbod.cusid}&ctype=${cusbod.ctype}&pageNum=1&csnum=${cusbod.csnum}&pageNum=1&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}'"/>
														</c:when>
														<c:when test="${cusbod.ctype eq 'R'}">
															<c:set var="trdata" value="location.href='/review/content?cusid=${cusbod.cusid}&ctype=${cusbod.ctype}&pageNum=1&csnum=${cusbod.csnum}&pageNum=1&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}'"/>
														</c:when>
														<c:when test="${cusbod.ctype eq 'T'}">
															<c:set var="trdata" value="location.href='/talk/content?cusid=${cusbod.cusid}&ctype=${cusbod.ctype}&pageNum=1&csnum=${cusbod.csnum}&pageNum=1&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}'"/>
														</c:when>
														<c:when test="${cusbod.ctype eq 'Q'}">
															<c:set var="trdata" value="location.href='/qna/content?cusid=${cusbod.cusid}&ctype=${cusbod.ctype}&pageNum=1&csnum=${cusbod.csnum}&pageNum=1&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}'"/>
														</c:when>
													</c:choose>
                                    ...
                                    <ul class="pagination pagination-xs pagination-circle">
                                   		<c:if test="${pageMaker.prev eq true}">
	                                        <li class="page-item page-indicator">
	                                            <a class="page-link" href="/empath/list?pageNum=${pageMaker.startPage - 1}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}#board"><i class="icon-arrow-left"></i></a>
	                                        </li>
                                   		</c:if>
                                   		<c:forEach var="i" begin="${pageMaker.startPage}" end="${pageMaker.endPage}" step="1">
                                        	<li class="page-item ${pageMaker.cri.pageNum eq i ? 'active' : ''}"><a class="page-link" href="/empath/list?pageNum=${ i }&type=${ pageMaker.cri.type }&keyword=${ pageMaker.cri.keyword }">${i}</a></li>
                                   		</c:forEach>
                                   		<c:if test="${pageMaker.next eq true}">
	                                        <li class="page-item page-indicator">
	                                            <a class="page-link" href="/empath/list?pageNum=${pageMaker.endPage + 1}&type=${pageMaker.cri.type}&keyword=${pageMaker.cri.keyword}#board"><i class="icon-arrow-right"></i></a>
	                                        </li>
                                   		</c:if>
                                    </ul>
                                </nav>
